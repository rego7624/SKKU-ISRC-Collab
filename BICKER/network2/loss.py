...
class LossEG(nn.Module):
    ...
    def loss_adv(self, r_x_hat, D_act_hat, D_act):
        fm = 0
        for i in range(0, len(D_act)):
            fm += F.l1_loss(D_act_hat[i],D_act[i])
        return -r_x_hat.mean() + fm * config.LOSS_FM_WEIGHT

    def loss_mch(self, e_hat, W_i):
        return F.l1_loss(W_i.reshape(-1), e_hat.reshape(-1)) * config.LOSS_MCH_WEIGHT

    def forward(self, x, x_hat, r_x_hat, e_hat, W_i, D_act_hat, D_act):
        if self.gpu is not None:
            ...
            for i in range(0, len(D_act)):
                D_act_hat[i] = D_act_hat[i].cuda(self.gpu)
                D_act[i] = D_act[i].cuda(self.gpu)

        cnt = self.loss_cnt(x, x_hat)
        adv = self.loss_adv(r_x_hat, D_act_hat, D_act)
