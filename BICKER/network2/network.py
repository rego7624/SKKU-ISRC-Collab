...
import torch.distributions as N
from torch.nn import functional as F

from .components import ResidualBlock, AdaptiveResidualBlock, ResidualBlockDown, ResidualBlockDownFirst, AdaptiveResidualBlockUp, SelfAttention, LastProcess
import config
...
class Embedder(nn.Module):
    ...
    def __init__(self, gpu=None):
        ...
        self.conv1 = ResidualBlockDownFirst(6, 64)
    ...
    def forward(self, x, y):
        ...
        # Vectorize
        out = self.pooling(out).view(-1, config.E_VECTOR_LENGTH)
...
class Discriminator(nn.Module):
    def __init__(self, training_videos, gpu=None):
        ...
        self.W = nn.Parameter(torch.rand(512, 138374).normal_(0.0, 0.02))
    ...
    def forward(self, x, y, i, e_hat):
        ...
        # Add noise
        #noise = N.Normal(loc=0, scale=1)
        #out = out + 0.3*noise.sample(out.shape)
        ...
        #print("w_0" + str(torch.mm(self.w_0.t(),self.w_0)))
        #print("e_hat" + str(torch.mm(e_hat*0.05,e_hat.t())[0]))
        #print("w_0" + str(torch.mm(self.W[:,i].t(),self.w_0)))
              
        _W_i = e_hat.unsqueeze(-1) * 0.1#0.05
        ...
        return out, [out_2, out_3, out_4, out_6]
